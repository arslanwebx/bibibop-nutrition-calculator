...
for(const viewport of [{name:"desktop",width:1440,height:1000},{name:"mobile",width:390,height:844}]){
  ...
  await page.goto("http://127.0.0.1:3005/",{waitUntil:"networkidle"});const calculatorWidth=await page.locator(".calculator-section > .shell").evaluate(node=>node.getBoundingClientRect().width);const categoryHeaders=await page.getByRole("columnheader",{name:"Category",exact:true}).count();const tableCaption=await page.locator(".nutrition-table-caption").textContent();const tableScroll=await page.locator(".nutrition-table-scroll").evaluate(node=>node.scrollWidth-node.clientWidth);const captionOffset=await page.evaluate(()=>{const caption=document.querySelector(".nutrition-table-caption")?.getBoundingClientRect();const wrap=document.querySelector(".table-wrap")?.getBoundingClientRect();return caption&&wrap?Math.abs((caption.left+caption.width/2)-(wrap.left+wrap.width/2)):null});const showcaseCards=await page.locator(".showcase-card").count();const showcaseColumns=await page.locator(".showcase-grid").evaluate(node=>getComputedStyle(node).gridTemplateColumns.split(" ").length);const allCalculatorsHref=await page.locator(".showcase-all-link").getAttribute("href");
