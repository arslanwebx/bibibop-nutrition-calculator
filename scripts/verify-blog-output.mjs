...
for(const slug of slugs){
  ...
  const imageSources=[...html.matchAll(/(?:src|href)="(\/blog\/[^"]+\.(?:webp|svg|jpg))"/g)].map(match=>path.join("public",match[1]));for(const image of imageSources)assert(fs.existsSync(image),`${slug} references missing image ${image}`);
