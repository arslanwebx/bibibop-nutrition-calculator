...
export default function BlogPostTemplate({post,relatedPosts,children}:{post:BlogPostSummary;relatedPosts:BlogPostSummary[];children:React.ReactNode}){
  ...
  return <>
    ...
    <article className="shell blog-article">
      ...
      <header className="blog-article-header"><Link className="article-category" href={category.url}>{category.name}</Link><h1>{post.title}</h1><p className="lede">{post.excerpt}</p><p className="article-meta">Last updated <time dateTime={lastUpdated}>{lastUpdatedLabel}</time></p></header>
