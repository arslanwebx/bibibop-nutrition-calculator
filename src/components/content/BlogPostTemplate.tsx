...
export default function BlogPostTemplate({post,relatedPosts,children}:{post:BlogPostSummary;relatedPosts:BlogPostSummary[];children:React.ReactNode}){
  ...
  return <>
    ...
    <article className="shell blog-article">
      ...
      <header className="blog-article-header"><Link className="article-category" href={category.url}>{category.name}</Link><h1>{post.title}</h1><p className="lede">{post.excerpt}</p><div className="article-meta"><span>Researched and written by <Link href={authorConfig.path}>{authorConfig.name}</Link></span><span>Published <time dateTime={post.publishedDate}>{post.publishedDate}</time></span>{post.updatedDate&&<span>Updated <time dateTime={post.updatedDate}>{post.updatedDate}</time></span>}{post.dataSourceDate&&<span>Data checked <time dateTime={post.dataSourceDate}>{post.dataSourceDate}</time></span>}</div></header>
      ...
      <div className="blog-article-layout"><nav className="article-toc" aria-label="Table of contents"><strong>In this guide</strong><ol>{post.toc.map(item=><li key={item.id}><a href={`#${item.id}`}>{item.label}</a></li>)}</ol></nav><div className="blog-article-body">{children}
