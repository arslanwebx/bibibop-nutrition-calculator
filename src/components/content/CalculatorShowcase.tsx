...
export default function CalculatorShowcase(){return <section className="tools-showcase" aria-labelledby="more-calculators-heading"><div className="shell"><div className="showcase-heading"><p className="eyebrow">Helpful nutrition tools</p><h2 id="more-calculators-heading">More Nutrition Calculators</h2><p>Use focused tools for calories, macros, protein, carbohydrates, fat, and daily energy needs.</p></div><div className="showcase-grid">{featured.map(tool=><Link className="showcase-card" href={calculatorHref(tool.slug)} key={tool.slug}><div className="showcase-card-title"><ToolIcon type={tool.slug}/><h3>{tool.name}</h3></div><p>{tool.shortDescription}</p><span className="showcase-arrow" aria-hidden="true">→</span></Link>)}</div><Link className="button-secondary showcase-all-link" href="/nutrition-calculators/">View all nutrition calculators <span aria-hidden="true">→</span></Link></div></section>}
