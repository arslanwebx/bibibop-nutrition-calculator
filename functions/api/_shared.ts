...
export async function verifyTurnstile(token:string,secret:string,request:Request){if(!token)return false;const body=new FormData();body.set("secret",secret);body.set("response",token);const ip=request.headers.get("CF-Connecting-IP");if(ip)body.set("remoteip",ip);const result=await fetch("https://challenges.cloudflare.com/turnstile/v0/siteverify",{method:"POST",body});if(!result.ok)return false;const data=await result.json() as {success?:boolean};return data.success===true}
...
export async function sendEmail(env:Env,payload:{subject:string;html:string;text:string;replyTo:string}){const res=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${env.RESEND_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({from:env.CONTACT_FROM_EMAIL,to:[env.CONTACT_TO_EMAIL],subject:payload.subject,html:payload.html,text:payload.text,reply_to:payload.replyTo})});return res.ok}
export function configured(env:Env){return Boolean(env.RESEND_API_KEY&&env.CONTACT_TO_EMAIL&&env.CONTACT_FROM_EMAIL&&env.TURNSTILE_SECRET_KEY&&env.SITE_URL)}
